...
class RobustScanner(object):
    def __init__(self,model_dir,infer_len=50):
        ...
        self.start_end=len(vocab)
        self.pad_value=self.start_end+1
        

        LOG_INFO(f"Label len:{self.pos_max}")
        LOG_INFO(f"Infer Len:{self.infer_len}")
    ...
    def encoder(self):
        ...
        * defatult backbone : DenseNet121 **changeable
        ...
        enc=backbone.output
        # enc 
        enc=tf.keras.layers.Conv2D(self.enc_filters,kernel_size=3,activation="relu",padding="same")(enc)
    ...
    def seq_decoder(self):
        ...
        embedding=tf.keras.layers.Embedding(self.pad_value+1,self.enc_filters)(gt)
    ...
    def porcess_data(self,img,boxes):
        ...
        for box in tqdm(boxes):
            ...
            crop=np.ones((h,w))
            crop[y_min:y_max,x_min:x_max]=img[y_min:y_max,x_min:x_max]
            crop=255-crop
